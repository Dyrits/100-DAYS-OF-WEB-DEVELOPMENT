...
const error = {
    ...
    clear: (session) => { session.error = null; }
}
...
const validate = {
    ...
    user: async (body, session) => {
        ...
        let duplicate;
        if (!validate._email(email, confirmation) || !validate._password(password) || (duplicate = !await validate._uniqueness(email))) {
            const message = duplicate ? "Email is already in use!" : "Invalid email or password!";
            error.create(email, confirmation, password, message, session, "signup");
        } else { error.clear(session); }
        await session.save();
    }
};
...
const service = {
    users: {
        ...
        save: async (body, session) => {
            ...
            if (session.signup) { return false; }
        ...
        authenticate : async (body, session) => {
            ...
            !authenticated && error.create(email, null, password, "Invalid email or password!", session, "login");
            await session.save();
        }
    }
};
